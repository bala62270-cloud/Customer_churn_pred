...
# 7. Save model
os.makedirs("/content/drive/MyDrive/Cultus project output", exist_ok=True)
joblib.dump(best, "outputs/best_model.joblib")

# 8. SHAP analysis
# Ensure all features are float type for SHAP, as its internal C extensions might expect float64
X_train_shap = X_train.astype(float)
X_test_shap = X_test.astype(float)

explainer = shap.Explainer(best, X_train_shap)
# Disable additivity check to prevent ExplainerError due to minor numerical discrepancies
...
for i, idx in enumerate(selected_indices):
    # Use X_test_shap for individual explanations as well
    sv = shap_values[idx]
    ...
    # Pass base_values and values explicitly
...
with open("/content/drive/MyDrive/Cultus project output/report.md", "w", encoding="utf-8") as f:
    f.write(report_md)

print("All outputs saved in 'outputs/' folder. Files: ", os.listdir("/content/drive/MyDrive/Cultus project output"))
